...
!!
!! \b Version: LG simulations. Very similar to cubep3m.
    ...
    if (rank == 0) then
       ...
       if (id_str /= "dmdens_cic") then
          dens_file=trim(adjustl(dir_dens))// &
               trim(adjustl(nz_str))//"rho_"//trim(adjustl(id_str))//".dat"
       else
          dens_file=trim(adjustl(dir_dens))// &
               trim(adjustl(nz_str))//trim(adjustl(id_str))//".dat"
       endif
